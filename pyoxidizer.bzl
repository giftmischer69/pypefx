...
def make_exe(dist):
    ...
    python_config.run_module = "pypefx"
    ...
    # exe.add_python_resources(exe.pip_install(["pypefx"]))
...
def make_install(exe):
    ...
    defaults_yaml = glob(["/defaults.yaml"])
    files.add_manifest(defaults_yaml)
...
register_target("", make_msi, depends=["exe"])
